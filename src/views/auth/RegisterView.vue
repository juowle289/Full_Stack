...
// Đăng ký bằng Google dùng chung 1 luồng với đăng nhập Google: backend tự tạo
// tài khoản mới (role Reader) nếu email chưa tồn tại - xem GOOGLE_LOGIN_SETUP.md.
...
function showGoogleMessage() {
  ...
  if (!googleClientId) {
    message.value = 'Đăng ký Google chưa được cấu hình VITE_GOOGLE_CLIENT_ID trong .env.'
    return
  }

  message.value = 'Không tải được nút đăng ký Google (mất mạng hoặc script bị chặn). Vui lòng thử lại.'
}

// Callback Google gọi khi người dùng chọn tài khoản thành công - backend sẽ tự tạo
// tài khoản Reader mới nếu email này chưa tồn tại trong hệ thống, sau đó trả JWT luôn.
async function handleGoogleCredential(response) {
  ...
  try {
    ...
    if (err.response?.status === 404) {
      message.value = 'Backend chưa hỗ trợ đăng ký/đăng nhập Google (thiếu API /api/identity/auth/google-login).'
    } else {
      message.value = err.response?.data?.message || 'Đăng ký Google thất bại. Vui lòng thử lại hoặc dùng Email/Mật khẩu.'
    }
    console.error(err.response || err)
  } finally {
    loading.value = false
